...
```
## Study Group: Neshma, Stan, and Hertz
```
...
```
##We changed the codes a bit to see whether it gives us more a correlation. 
```
...
mean(correct_rate)
```
We increased the K value from 15 to 19 in order to determine a correlation. However, it seems that this isn't a well determinant of the correlation between household incomes, household costs and the neighborhoods/boroughs. We chose to look at the household incomes in the boroughs to see whether the code is a good determinant of how the household incomes of people correlates with the boroughs they reside in.
```